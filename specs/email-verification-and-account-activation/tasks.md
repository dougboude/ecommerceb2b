...
- [x] 1.1 Add `EmailVerificationToken` to `marketplace/models.py`
...
- [x] 1.2 Generate and apply additive migration for `EmailVerificationToken`
...
- [x] 1.3 Register `EmailVerificationToken` in `marketplace/admin.py`
...
- [x] 2.1 Add `_send_verification_email(request, user)` helper in `marketplace/views.py`
...
- [x] 2.2 Update `signup_view` in `marketplace/views.py`
...
- [x] 3.1 Add `verify_email` view in `marketplace/views.py`
  - Renders `templates/registration/email_verify.html`
  - Accessible to unauthenticated users (no `@login_required`)
  - _Requirements: 3.6_

- [x] 3.2 Add `verify_email_confirm` view in `marketplace/views.py`
...
- [x] 3.3 Add new URL patterns to `marketplace/urls.py`
...
- [x] 4.1 Update `MarketplaceLoginView.form_valid()` in `marketplace/views.py`
...
- [x] 4.2 Fix `UserManager.create_superuser` in `marketplace/models.py` to prevent bootstrap lockout
...
- [x] 5.1 Add `resend_verification` view in `marketplace/views.py`
...
- [x] 6.1 Create `templates/registration/verification_email_subject.txt`
  - Content: `Confirm your NicheMarket account` (single line, no trailing newline)
  - _Requirements: 2.4_

- [x] 6.2 Create `templates/registration/verification_email_body.txt`
...
- [x] 7.1 Update `templates/registration/email_verify.html` (existing stub)
...
- [x] 7.2 Create `templates/registration/email_verify_expired.html`
...
- [x] 7.3 Create `templates/registration/email_verify_used.html`
  - Extend `base.html`; use `.auth-form` container
...
- [x] 7.4 Create `templates/registration/resend_verification.html`
  - Extend `base.html`; use `.auth-form` container
...
- [x] 8.1 Audit `settings.py` for `EMAIL_BACKEND`; add if not present
...
- [x] 9.1 Create `marketplace/tests/test_email_verification.py`
...
- [x] 9.2 Write `test_signup_does_not_autologin`
...
- [x] 9.3 Write `test_signup_creates_token`
  - POST to `/signup/` with valid data
  - Assert `EmailVerificationToken.objects.filter(user=<new_user>).count() == 1`
  - _Requirements: 2.1_

- [x] 9.4 Write `test_signup_sends_verification_email`
  - POST to `/signup/` with valid data
...
- [x] 9.5 Write `test_verify_valid_token_activates_and_logs_in`
...
- [x] 9.6 Write `test_verify_expired_token_shows_expired_page`
...
- [x] 9.7 Write `test_verify_used_token_shows_used_page`
  - Create token with `used_at` already set
  - GET `/verify-email/<token>/`
  - Assert response renders `email_verify_used.html`
  - _Requirements: 3.4, 6.2_

- [x] 9.8 Write `test_verify_nonexistent_token_returns_404`
  - GET `/verify-email/<random-uuid>/`
  - Assert 404 response
  - _Requirements: 3.5_

- [x] 9.9 Write `test_login_blocked_for_unverified_user`
...
- [x] 9.10 Write `test_login_allowed_for_verified_user`
...
- [x] 9.11 Write `test_resend_sends_new_token`
...
- [x] 9.14 Write `test_token_is_valid_property`
...
- [x] 9.15 Write `test_email_send_failure_does_not_break_signup`
...
- [x] 10.2 Run all migration validate scopes
  ```
...
- [ ] 10.3 (manual — skipped in automated execution) Manual smoke test (dev environment)
...
- [x] 10.4 Confirm scope boundaries — no unrelated features included
...
- [x] 10.5 Update `specs/SPEC_ORDER.md` status to `REQ, DES, TASK, EXEC`
