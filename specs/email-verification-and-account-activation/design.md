...
The token UUID is generated by default and never exposed to the user except in the email URL. `expires_at` is set automatically on first save. `revoked_at` is set when a resend or new signup invalidates a prior token — the row is kept for audit purposes (never hard-deleted).
...
1. Revoke prior unused tokens for the user (preserves audit history): `EmailVerificationToken.objects.filter(user=user, used_at=None, revoked_at=None).update(revoked_at=now())`
...
def verify_email_confirm(request, token):
    ...
    if not obj.is_valid:  # expired (used_at is None, revoked_at is None, but past expiry)
...
**Why double-check inside the transaction:** The outer `get()` and the inner `select_for_update().get(pk=..., used_at=None, revoked_at=None)` together prevent two concurrent requests from both reaching the activation block. The inner `.get()` filters on `used_at=None, revoked_at=None` — if another request won the race, the `DoesNotExist` branch renders the "already verified" page cleanly. The `backend` kwarg is required when calling `login()` outside of `authenticate()`.
...
### Superuser Bootstrap Fix

`UserManager.create_superuser` does not currently set `email_verified=True`. Without this fix, any superuser created via `manage.py createsuperuser` will be locked out of admin on first login. Fix:
...
def create_superuser(self, email, password=None, **extra_fields):
    ...
    extra_fields.setdefault("email_verified", True)  # prevent bootstrap lockout
    return self.create_user(email, password, **extra_fields)
```

This uses `setdefault` so callers can still explicitly pass `email_verified=False` if needed. Existing superusers in the database are not affected by this code change — they must be manually verified via shell or admin if already created.
