...
- [x] 1.1 Audit `marketplace/forms.py` `SignupForm` for any surviving `role` field assignment
...
- [x] 1.2 Audit `templates/registration/signup.html`; fix if violations found
...
- [x] 1.3 Audit `templates/registration/login.html` for role-based language
  - No changes expected; confirm with scanner output
  - _Requirements: 2.1, 2.2_

### Group 2 — Navigation Derolification

- [x] 2.1 Audit `templates/includes/_navbar.html` for all `user.role` references
...
- [x] 2.2 Replace role-conditional nav with role-agnostic nav in `templates/includes/_navbar.html`
...
- [x] 2.3 Audit `templates/base.html` for any role-conditional blocks
  - Remove any `{% if user.role ... %}` wrapper found at the base level
  - _Requirements: 2.2, 3.3_

### Group 3 — Dashboard Derolification

- [x] 3.1 Audit `templates/marketplace/dashboard.html` for role-based headings and section conditionals; fix if violations found
...
- [x] 3.2 Verify `templates/marketplace/dashboard.html` target state; update only what deviates
...
- [x] 3.3 Audit `dashboard` view in `marketplace/views.py`; fix surviving role branches if found
...
- [x] 4.1 Audit `profile` view in `marketplace/views.py`; fix if role usage found
...
- [x] 4.2 Audit `templates/marketplace/profile.html`; fix if violations found
...
- [x] 5.1 Audit `templates/marketplace/supply_lot_form.html`; fix if violations found
...
- [x] 5.2 Audit `templates/marketplace/demand_post_form.html`; fix if violations found
...
- [x] 5.3 Audit `templates/marketplace/supply_lot_list.html` and `templates/marketplace/demand_post_list.html`; fix if violations found
...
- [x] 5.4 Audit `templates/marketplace/supply_lot_detail.html` and `templates/marketplace/demand_post_detail.html`; fix if violations found
  - Remove any role-identity labels from detail headings or meta if found
  - _Requirements: 2.1_

### Group 6 — Remaining Python View Audit

- [x] 6.1 Audit all view functions in `marketplace/views.py` for surviving `user.role` display references; fix if found
...
- [x] 6.2 Confirm `Role` enum is no longer imported in `marketplace/views.py` or `marketplace/forms.py`
...
- [x] 7.1 Create `marketplace/migration_control/ui_compliance.py`
...
- [x] 7.2 Add `validate_ui_language()` to `marketplace/migration_control/parity.py`
  - Delegates to `TemplateLanguageComplianceScanner().scan()`
  - Returns `ValidationResult`
  - _Requirements: 6.6_

- [x] 7.3 Add `"ui"` scope to `marketplace/management/commands/migration_validate.py`
...
- [x] 7.4 Run `manage.py migration_validate --scope ui --fail-on-error`
...
- [x] 8.1 Review all templates for `{% if user.role is not None %}`, `{% if user.role %}`, `{% with role=user.role %}` patterns
  - If any found: add to violation list in scanner and fix before proceeding
  - _Requirements: 7.2, 7.3_

- [x] 8.2 Confirm no template uses `hasattr(user, 'role')` or equivalent Python-in-template guard
  - Django templates don't support `hasattr` directly, but check for custom template tags or `{% if user|has_attr:"role" %}` patterns
  - _Requirements: 7.2_

- [x] 8.3 Document any intentional exception where role-adjacent language is preserved
...
- [x] 9.1 Create `marketplace/tests/test_ui_derolification.py`
...
- [x] 9.2 Write `test_signup_renders_no_role_field`
...
- [x] 9.3 Write `test_signup_heading_is_create_account`
...
- [x] 9.4 Write `test_navbar_shows_supply_and_demand_for_all_authenticated_users`
...
- [x] 9.5 Write `test_dashboard_heading_is_not_role_labeled`
  - Log in as any user
  - GET dashboard URL
...
- [x] 9.6 Write `test_profile_shows_both_listing_sections`
...
- [x] 9.7 Write `test_compliance_scanner_zero_violations`
...
- [x] 9.8 Identify and retire stale migration-era tests that asserted role-based UI behavior
...
- [x] 10.3 Confirm scope boundaries — no unrelated features included
...
- [x] 10.4 Update `specs/SPEC_ORDER.md` status to `REQ, DES, TASK, EXEC`
